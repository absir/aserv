 ...
 */
public class TAdapterPush {

    protected static final Logger LOGGER = LoggerFactory.getLogger(TAdapterPush.class);
    ...
    public void bind(SocketAdapter adapter) {
        receiveCallbacks = adapter.getReceiveCallbacks();
        receiveCallbacks.put(PUSH_MAP_CALLBACK, new KernelLang.ObjectEntry<SocketAdapter.CallbackAdapter, SocketAdapter.CallbackTimeout>(new SocketAdapter.CallbackAdapter() {
            @Override
                    ...
                    for (Map.Entry<String, Integer> entry : nameMapCallbackIndex.entrySet()) {
                        ...
                        if (ifaceProcessFunction != null) {
                            ...
                            receiveCallbacks.put(entry.getValue(), new KernelLang.ObjectEntry<SocketAdapter.CallbackAdapter, SocketAdapter.CallbackTimeout>(new SocketAdapter.CallbackAdapter() {
                                ...
                                public void doWith(SocketAdapter adapter, int offset, byte[] buffer) {
                                    try {
                                        processFunction.process(0, new TCompactProtocol(new TMemoryInputTransport(buffer, offset, buffer.length)), null, iface);
